...
router.post("/", async (req, res) => {
    try {
        ...
        const intent = body.intent || "news";
        const profile = body.profile;
        if (!user?.trim()) {
            return res.json({
                answer: "Halo! Saya ArkWork Agent. Saya bisa bantu ringkas berita migas, rekomendasi kerja, dan konsultasi langkah praktis. Coba: 'Rekomendasikan role untuk operator kilang dengan pengalaman 2 tahun di Balikpapan.'",
            });
        }
